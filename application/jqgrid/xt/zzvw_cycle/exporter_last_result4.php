...
class xt_zzvw_cycle_exporter_last_result extends exporter_excel{
	...
	protected $strIds = '';
	protected $result_header = array('total', 'pass', 'fail', 'n/a', 'n/s', 'n/t', 'ne', 'others', 'pass_rate', 'finish_rate');
	protected $mid = array();
...
//Array ( [db] => xt [table] => zzvw_cycle [export_type] => last_result [id] => array(10,9) [real_table] => cycle ) 
		...
		$res = $this->db->query("SELECT cycle.*, prj.name as prj, rel.name as rel FROM cycle left join prj on cycle.prj_id=prj.id left join rel on cycle.rel_id=rel.id WHERE cycle.id IN ({$this->strIds})");
		while($cycle = $res->fetch())
			$this->cycleInfo[$cycle['id']] = $cycle;
		$this->cycleInfo[-1] = array('id'=>-1, 'name'=>'Final Cycle', 'prj'=>'Merged Project', 'rel'=>'Merged Release');
		
		$res = $this->db->query("SELECT distinct compiler.id, compiler.name FROM compiler left join cycle on compiler.id=cycle.compiler_id where cycle.id in ({$this->strIds})");
		while($cycle = $res->fetch())
			$this->compiler[$cycle['id']] = $cycle;
			
		$res = $this->db->query("SELECT distinct build_target.id, build_target.name FROM build_target left join cycle on build_target.id=cycle.build_target_id where cycle.id in ({$this->strIds})");
		while($cycle = $res->fetch())
			$this->build_target[$cycle['id']] = $cycle;
			
		$res = $this->db->query("SELECT distinct test_env.id, test_env.name FROM test_env left join cycle_detail on test_env.id=cycle_detail.test_env_id where cycle_detail.cycle_id in ({$this->strIds})");
		while($cycle = $res->fetch())
		...
		if($this->cycleCount > 1){
			$res = $this->db->query("SELECT distinct cycle.prj_id as id, prj.name FROM cycle left join prj on cycle.prj_id=prj.id WHERE cycle.id IN ({$this->strIds})");
			while($row = $res->fetch()){
				$this->prj[$row['id']] = $row;
			}
			if(count($this->prj) == 1)
				$this->prjMerge = true;
				$this->cycleInfo[-1]['prj'] = $row['name'];
		}
	...
	protected function getMidData(){
		$mid1_unmerged = $this->getCodecStreamDetailMidData();
		$data1_detail = $this->handleMidData($mid1_unmerged);
		$data1_stat = $this->handleMidData($mid1_unmerged, true);
		
		$mid2_unmerged = $this->getNomalCaseDetailMidData();
		$data2_detail = $this->handleMidData($mid2_unmerged);
		$data2_stat = $this->handleMidData($mid2_unmerged, true);
		if($this->prjMerge){
			$mid1 = $this->getCodecStreamDetailMidData($this->prjMerge);
			$data1_merged_detail = $this->handleMidData($mid1);
			$data1_merged_stat = $this->handleMidData($mid1, true);
			$mid2 = $this->getNomalCaseDetailMidData($this->prjMerge);
			$data2_merged_detail = $this->handleMidData($mid2);
			$data2_merged_stat = $this->handleMidData($mid2, true);
		}
		$mid = array('unmerged'=>array('stream'=>$mid1_unmerged, 'stream_detail'=>$data1_detail, 'stream_stat'=>$data1_stat, 
			'normal'=>$mid2_unmerged, 'normal_detail'=>$data2_detail, 'normal_stat'=>$data2_stat));
		if($this->prjMerge){
			$mid['merged'] = array('stream'=>$mid1, 'stream_detail'=>$data1_merged_detail, 'stream_stat'=>$data1_merged_stat, 
				'normal'=>$mid2, 'normal_detail'=>$data2_merged_detail, 'normal_stat'=>$data2_merged_stat);
		}
		$this->mid = $mid;
		
		$mid1 = $this->mid['unmerged']['stream_stat'];
		$mid2 = $this->mid['unmerged']['normal_stat'];
		$data = array_merge($mid1, $mid2);
		if($this->prjMerge){
			$mid1 = $this->mid['merged']['stream_stat'];
			$mid2 = $this->mid['merged']['normal_stat'];
			$data = array_merge($data, $mid1, $mid2);
		}
		$this->mid_data['stat'] = $data;
		
		$mid1 = $this->mid['unmerged']['stream_detail'];
		$mid2 = $this->mid['unmerged']['normal_detail'];
		$data = array_merge($mid1, $mid2);
		if($this->prjMerge){
			$mid1 = $this->mid['merged']['stream_detail'];
			$mid2 = $this->mid['merged']['normal_detail'];
			$data = array_merge($data, $mid1, $mid2);
		}
		$this->mid_data['detail'] = $data;
	}
	
	public function setOptions($jqgrid_action){
		if(count($this->testcase_type) == 1){
			switch($this->testcase_type){
...
// print_r($this->params['sheets']);
	...
	protected function getCycleSummarySheet($jqgrid_action){
		...
		$sheet['groups'] = array(
			...
			array('index'=>'build_target', 'subtotal'=>$subTotal),
		);
		if(count($this->test_env) == 1){
			$sheet['header']['rows'][0][6]['hidden'] = true;
		}
	...
	protected function getCodecStreamDetailMidData($merged = false){
		$mid = array();
		$mainFields = "cycle.prj_id, cycle.compiler_id, cycle.build_target_id, prj.name as prj, codec_stream.codec_stream_format_id, ".
			" codec_stream_format.name as testcase_module, detail.codec_stream_id, detail.test_env_id,".
			" GROUP_CONCAT(CONCAT(detail.testcase_id, ':', detail.result_type_id)) AS results";
		if(!$merged)
		...
		if($merged){
			$subSql = "(SELECT cycle_detail.cycle_id, cycle.prj_id, cycle.compiler_id, cycle.build_target_id, cycle_detail.result_type_id, ".
				" cycle_detail.test_env_id, cycle_detail.codec_stream_id, cycle_detail.testcase_id ".
				" FROM cycle_detail ".
				" left join cycle on cycle_detail.cycle_id=cycle.id".
				" WHERE cycle_id in ({$this->strIds}) AND codec_stream_id>0 AND ".
				...
				" FROM cycle_detail ".
				" left join cycle on cycle_detail.cycle_id=cycle.id".
				" where cycle_id IN ({$this->strIds}) AND codec_stream_id>0".
		...
		$sql = "SELECT $mainFields".
			...
			" where detail.cycle_id in ({$this->strIds}) and detail.codec_stream_id>0 ".
			" GROUP BY detail.cycle_id, cycle.prj_id, cycle.compiler_id, cycle.build_target_id, detail.test_env_id, detail.codec_stream_id".
			" ORDER BY cycle_id, prj, testcase_module, test_env_id, codec_stream_id";
// if($merged)
// print_r($sql);
		...
		while($row = $res->fetch()){
			...
			$mid[$row['cycle_id']][$row['prj']][$row['compiler_id']][$row['build_target_id']][$row['testcase_module']][$row['test_env_id']][$row['codec_stream_id']] = $stream_result;//array($results, $stream_result);
		}
// print_r($mid);
		return $mid;
	}
	
	protected function getNomalCaseDetailMidData($merged = false){
		$mid = array();
		$mainFields = "cycle.prj_id, cycle.compiler_id, cycle.build_target_id, prj.name as prj, testcase.testcase_module_id, ".
			" testcase_module.name as testcase_module, detail.test_env_id, detail.testcase_id, detail.result_type_id, detail.test_env_id";
			
		if(!$merged)
			$mainFields .= ", cycle_id";
			
		$subSql = "cycle_detail";
		$subFields = "cycle_id, prj_id, compiler_id, build_target_id, result_type_id, cycle_detail.test_env_id, codec_stream_id, testcase_id";
		...
		$sql = "SELECT $mainFields".
			...
			" left join prj on cycle.prj_id=prj.id".
			" where detail.cycle_id in ({$this->strIds}) AND detail.codec_stream_id=0".
			" ORDER BY cycle_id, prj, testcase_module, test_env_id, testcase_id";
// if($merged)
// print_r($sql);
		$res = $this->db->query($sql);
		while($row = $res->fetch()){
			if($merged)
				$row['cycle_id'] = -1;
			$mid[$row['cycle_id']][$row['prj']][$row['compiler_id']][$row['build_target_id']][$row['testcase_module']][$row['test_env_id']][$row['testcase_id']] = $row['result_type_id'];
		}
		return $mid;
	}

	protected function handleMidData($mid, $stat = false){
		if(empty($mid) || count($mid) == 0)
			return array();
		$i = 0;
		$data = array();
		foreach($mid as $cycle_id=>$cycle_data){
			foreach($cycle_data as $prj=>$prj_data){
				foreach($prj_data as $compiler_id=>$compiler_data){
					foreach($compiler_data as $build_target_id=>$build_target_data){
						foreach($build_target_data as $testcase_module=>$module_data){
							foreach($module_data as $test_env_id=>$test_env_data){
								$data[$i] = array('cycle_id'=>$cycle_id, 'name'=>$this->cycleInfo[$cycle_id]['name'], 'prj'=>$prj, 'rel'=>$this->cycleInfo[$cycle_id]['rel'], 
									'compiler'=>$this->compiler[$compiler_id]['name'], 'build_target'=>$this->build_target[$build_target_id]['name'],
									'module'=>$testcase_module, 'test_env'=>$this->test_env[$test_env_id]['name']);
								if($stat){
									$data[$i]['total'] = $data[$i]['pass'] = $data[$i]['fail'] = $data[$i]['n/a'] = $data[$i]['n/s'] = $data[$i]['n/t'] = 
										$data[$i]['ne'] = $data[$i]['others'] = 0;
								}
								foreach($test_env_data as $testcase_id=>$result_type_id){
									...
									if($stat){
										...
										$data[$i][$testcase_id] = $result_type_id;
									}
								}
								if($stat){
...
// print_r($sql);
	...
	protected function getPrjModuleData($order = 'prj'){
		$data = array();
// print_r($this->mid_data['stat']);
		foreach($this->mid_data['stat'] as $d){
			$id = $d['prj'].'_'.$d['compiler'].'_'.$d['build_target'].'_'.$d['test_env'].'_'.$d['module'];
			if(!isset($data[$id]))
				$data[$id] = $d;
			foreach($this->result_header as $r){
				if($d['cycle_id'] == -1)
					$data[$id]['final_'.$r] = $d[$r];
				else
					$data[$id]['cycle_'.$d['cycle_id'].'_'.$r] = $d[$r];
			}
		}
// print_r($data);
		return $data;
