...
class cycle_contrast_export extends base_report{
	private $data = array();
	...
	protected function _writeHeader($worksheetIndex, $setWidth = true, $mergeColumns = array()){
		...
		while($row = $res->fetch()){
			...
			if($info = $info_res->fetch())
				$title1[] = array('label'=>$row['name'], 'width'=>300, 'index'=>$row['id'].'_result');
	...
	protected function getData($worksheetIndex){//worksheetindex=1时没什么问题
		...
		if ($worksheetIndex == 1){
			...
			while($row = $res->fetch()){
				...
				$this->data[1][$row['tc_id']][$row['cycle_id'].'_result'] = $row['result_type'];
				...
				foreach($this->params['results'] as $val){
					$result_type = strtolower($row['result_type']);
					if(strtolower($val) == $result_type){
						if (!isset($result[$row['module_id']][$row['cycle_id'].'_'.$result_type]))
							$result[$row['module_id']][$row['cycle_id'].'_'.$result_type] = 0;
						// if (empty($result[$row['module_id']][$row['cycle_id'].'_'.$result_type]))
							// $result[$row['module_id']][$row['cycle_id'].'_'.$result_type] = 0;
							// //$this->data[0]['module'][$row['module_id']][$row['cycle_id'].'_'.$row['result_type']] = 0;
						$result[$row['module_id']][$row['cycle_id'].'_'.$result_type]++;
						$this->data[0]['module'][$row['module_id']][$row['cycle_id'].'_'.$result_type] = $result[$row['module_id']][$row['cycle_id'].'_'.$result_type];
					}
				}
				if(!isset($this->data[0]['module'][$row['module_id']][$row['cycle_id'].'_blank'])){
					$blank_sql = "SELECT count(*) as blank FROM cycle_detail detail LEFT join testcase ON detail.testcase_id=testcase.id".
						" WHERE result_type_id=0 AND detail.cycle_id=".$row['cycle_id']." AND testcase.testcase_module_id=".$row['module_id'];
					$blank_res = $this->params['db']->query($blank_sql);
					$data = $blank_res->fetch();
					$this->data[0]['module'][$row['module_id']][$row['cycle_id'].'_blank'] = $data['blank'];
				}
				if(!isset($this->data[0]['module'][$row['module_id']][$row['cycle_id'].'_total'])){
					$blank_sql = "SELECT count(*) as total FROM cycle_detail detail LEFT join testcase ON detail.testcase_id=testcase.id".
						" WHERE detail.cycle_id=".$row['cycle_id']." AND testcase.testcase_module_id=".$row['module_id'];
					$blank_res = $this->params['db']->query($blank_sql);
					$data = $blank_res->fetch();
					$this->data[0]['module'][$row['module_id']][$row['cycle_id'].'_total'] = $data['total'];
				}
