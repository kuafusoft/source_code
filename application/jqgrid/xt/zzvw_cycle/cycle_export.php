	...
	protected function _writeHeader($worksheetIndex, $setWidth = true, $mergeColumns = array()){
		...
		while($row = $res->fetch()){
			...
			foreach($this->params['results'] as $result){
				...
				$title0_1[] =  array('label'=>$result, 'width'=>80, 'index'=>$row['id'].'_'.$result, 'cases'=>$resultCount['cc'], 'style'=>$even_style);
			}
...
//print_r($tmp);						
	...
	private function writeCycle($worksheetIndex = 0){
		...
		$column = $this->columnHeaderStartColumn[$worksheetIndex];
		$this->nextRow[0]++;
		$this->writeCell($worksheetIndex, $this->nextRow[0], $column, "Prj:", 'total');
		$this->writeCell($worksheetIndex, $this->nextRow[0]+1, $column, "Os:", 'total');
		$column_start = $column;
		$flag = 1;
		foreach($this->data[0]['cycle'] as $prj=>$prj_v){
			$this->writeCell($worksheetIndex, $this->nextRow[0], $column + 1, $prj, 'total');
			foreach($prj_v as $os=>$os_v){
				$this->writeCell($worksheetIndex, $this->nextRow[0]+1, $column + 1, $os, 'total');
				foreach($os_v as $cycleInfo){
					$row = $this->nextRow[0] + 2;
		...
		for($i=$column_start+2; $i<=$column; $i++)
			$this->writeCell($worksheetIndex, $this->nextRow[0], $i, '', 'total');
		$this->mergeCells($worksheetIndex, $this->nextRow[0]+1, $column_start+1, $this->nextRow[0]+1, $column);
