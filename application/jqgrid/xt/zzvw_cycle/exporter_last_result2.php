...
// print_r($sql);
	...
	protected function getStreamSql($order = 'prj', $lastResult = false){
		...
		$sql = "SELECT prj.name as prj, codec_stream_format.name as module, cycle.prj_id, cycle.compiler_id,".
			" detail.cycle_id as cycle_id, codec_stream.codec_stream_format_id, ".
			" group_concat(concat(detail.codec_stream_id, ':', detail.result_type_id)) as result_type_id,".
			" count(distinct codec_stream_id) as cases".
			" FROM cycle ";
		$detail = 'cycle_detail';
		if($lastResult){
			$detail = "(".
				" SELECT cycle_id, codec_stream_id, result_type_id ".
				" FROM cycle_detail Z ".
				" WHERE cycle_id in ({$this->strIds}) ".
				" AND (codec_stream_id, testcase_id, cycle_id) in (".
					" SELECT codec_stream_id, testcase_id, max(cycle_id) ".
		...
		$sql .= " LEFT JOIN $detail detail ON cycle.id=detail.cycle_id";

		$sql .= " LEFT JOIN codec_stream ON detail.codec_stream_id=codec_stream.id".
			" LEFT JOIN prj ON cycle.prj_id=prj.id".
		...
		if(!$lastResult)
			$sql .= "cycle_id, ";
		$sql .= "prj_id, compiler_id, codec_stream_format_id";
		if($order == 'prj')
			$sql .= " ORDER BY prj ASC, module ASC, cycle_id, start_date ASC";
		else	
			$sql .= " ORDER BY module ASC, prj ASC, cycle_id, start_date ASC";
	...
	protected function handleStreamData($row, &$data, $lastResult = false){
		...
		if($lastResult)
			...
			$field = 'cycle_'.$row['cycle_id'].'_total';
		$data[$row['prj_id']][$row['compiler_id']][$row[$m]][$field] = $row[$f];
...
// print_r($streamResults);
		...
		foreach($streamResults as $stream_id=>$results){
			...
			if($lastResult)
				...
				$field = 'cycle_'.$row['cycle_id'].'_'.$name;
			if(!isset($data[$row['prj_id']][$row['compiler_id']][$row[$m]][$field]))
				$data[$row['prj_id']][$row['compiler_id']][$row[$m]][$field] = 0;
			$data[$row['prj_id']][$row['compiler_id']][$row[$m]][$field] ++;
		}
	}
...
// print_r($row);
	...
	protected function getCaseSql($order = 'prj', $lastResult = false){
		...
		$sql = "SELECT prj.name as prj, testcase_module.name as module, cycle.prj_id, cycle.compiler_id,".
			" detail.cycle_id as cycle_id, detail.result_type_id,".
			" count(distinct testcase_id) as cases".
			" FROM cycle ";
		$detail = 'cycle_detail';
		if($lastResult){
			$detail = "(".
		...
		if(!$lastResult)
			$sql .= "cycle_id, ";
		$sql .= "prj_id, compiler_id, module, result_type_id";
			if($order == 'prj')
				$sql .= " ORDER BY prj ASC, module ASC, cycle_id, start_date ASC";
			else	
				$sql .= " ORDER BY module ASC, prj ASC, cycle_id, start_date ASC";
	...
	protected function handleCaseData($row, &$data, $lastResult = false){
		...
		switch($row['result_type_id']){
			case RESULT_TYPE_PASS:
				...
				$data[$row['prj_id']][$row['compiler_id']][$row[$m]][$field.'_pass'] += $row[$f];
				break;
			case RESULT_TYPE_FAIL:
				if(!isset($data[$row['prj_id']][$row['compiler_id']][$row[$m]][$field.'_fail']))
					$data[$row['prj_id']][$row['compiler_id']][$row[$m]][$field.'_fail'] = 0;
				$data[$row['prj_id']][$row['compiler_id']][$row[$m]][$field.'_fail'] += $row[$f];
				break;
			default:
				if(empty($data[$row['prj_id']][$row['compiler_id']][$row[$m]][$field.'_others']))
					$data[$row['prj_id']][$row['compiler_id']][$row[$m]][$field.'_others'] = 0;
				$data[$row['prj_id']][$row['compiler_id']][$row[$m]][$field.'_others'] += $row[$f];
				break;
		}
