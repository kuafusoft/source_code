...
class logfileprocess{
	...
	//$cycleID not exists，create a new cycle; $cycleID exists, update the cycle
	function uploadLogForCycle($fileName, $params){
		$cycleID = '';
		if(isset($params['element']))
			$cycleID = $params['element'];
		$os_id = $params['os_id'];
		$upload_type = $params['upload_type'];
		switch($os_id){
			case 9://mqx
				$this->generateMqxCycle($fileName, $os_id, $upload_type, $cycleID);
				break;
			case 7://android jb42--codec
				$this->generateCodecCycle($fileName, $os_id, $upload_type, $cycleID);
				break;
			case 11://psdk
				$this->generatePSDKCases($fileName, $os_id, $upload_type, $cycleID);
				break;
			default:
				//$this->uploadLog($fileName, $cycleID);
				break;
		}   
	}
	
	private function generateMqxCycle($fileName, $os, $upload_type, $cycleID){
		require_once('mqx_import.php');
		require_once('mqx_log.php');
		$fileName = $this->handleZipFile($fileName);
		$log = new mqx_log();
		$log->root($fileName);
		$data = $log->analyze();
		$creater = 'b43100';
		$dsn = array('dbname'=>'xt', 'host'=>'localhost', 'username'=>'root', 'password'=>'NewLife11');
		$import = new mqx_import($dsn, $data, $creater);//creater:zhang jian
		$import->import($cycleID, $upload_type);
		return "success";
	}
	...
	private function handleZipFile($fileName){
		...
		if (stripos(strtolower($relFileName), '.zip') !== false){
			// analyze the file
			//查看目录是否存在，如果存在，则删除，然后重新上传，重新生成和update是不一样的。
			$fName = dirname($fileName).'/'.basename($fileName, '.zip');
			if(!file_exists($fName)){
				if(PHP_OS == 'Linux'){
					$cmd = 'unzip -o '.$fileName.' -d '.dirname($fileName);
					...
					$cmd = 'wzunzip '.$fileName.' '.dirname($fileName).' -d';
					$line = exec($cmd, $output, $retVal);
					if ($retVal){ // failed
						print_r("retVal = $retVal");
						return;
	...
	private function generateCodecCycle($fileName, $os_id, $upload_type, $cycleID){//excel
		require_once('codec_import.php');
		$creater = 'b42575'; // Dong Fei, 应该是上传人的id才对。
		if(stripos($fileName, ".xlsx"))
			$baseName = basename($fileName, ".xlsx");
		else if(stripos($fileName, ".xls"))
			$baseName = basename($fileName, ".xls");
		$data = explode("-", $baseName);
		$board_type = $data[0];
		$chip = $data[1];
		$compiler = $data[2];
		$date = $data[3];
		$rel = $data[4];
		$dsn = array('dbname'=>'xt', 'host'=>'localhost', 'username'=>'root', 'password'=>'NewLife11');
		$config_file = PUBLIC_PATH.'/tools/jellybean_codec.config.php';
		$codec_import = new codec_import($dsn);
		$data = $codec_import->parseCodec($config_file);

		$creater_id = $owner_id = $codec_import->getElementId('useradmin.users', array('username'=>$creater));	
		$board_type_id = $codec_import->getElementId('board_type', array('name'=>$board_type));
		//$os_id = $codec_import->getElementId('os', array('id'=>$os));
		$chip_id = $codec_import->getChipId($chip);
		$res = $this->db->query("SELECT name FROM os WHERE id=".$os_id);
		$info = $res->fetch();
		$os = $info['name'];
		$name = $codec_import->generateProjectName($chip, $board_type, $os);
		$prj = compact('name', 'chip_id', 'board_type_id', 'os_id', 'creater_id', 'owner_id');
		$prj_id = $codec_import->getElementId('prj', $prj);
		
		$codec_import->import_testcase($data, array($prj_id));
		if($upload_type == 2){// for cycle
			if(!empty($cycleID))
				$cycleInfo = array('id'=>$cycleID);
			else
				$cycleInfo = array('prj'=>$name, 'rel'=>$rel, 'creater'=>$creater_id, 'start_date'=>$date, 'end_date'=>$date, 'testcase_type_id'=>'TESTCASE_TYPE_CODEC');			
			$codec_import->import_testresult($cycleInfo, $data);
		}
	}
	
	private function generatePSDKCases($fileName, $os_id, $upload_type, $cycleID){//html
		require_once('testresult_import.php');
		require_once('psdk_import.php');
		
		$creater = 'b11133'; // Bill Yuan ;
		$dsn = array('dbname'=>'xt', 'host'=>'localhost', 'username'=>'root', 'password'=>'NewLife11');
		$import = new psdk_import($dsn);
		$data = $import->parseHtmlPSDK($fileName);
		$creater_id = $owner_id = $import->getElementId('useradmin.users', array('username'=>$creater));
		$prj_ids = array();
		$res = $this->db->query("select id from prj where os_id=".$os_id." and isactive=1");
		while($info = $res->fetch()){
			$prj_ids[] = $info['id'];
		}
		$import->import_testcase($data, $prj_ids);
		if($upload_type == 2){// for cycle
print_r("need uniform the format of the log file");
			// $prj_id = '';//最好能上传
			// if(!empty($cycleID))
				// $cycleInfo = array('id'=>$cycleID, 'prj'=>$prj, 'rel'=>$rel, 'creater'=>$creater_id, 'start_date'=>$date, 'end_date'=>$date, 'testcase_type_id'=>'TESTCASE_TYPE_PSDK');
			// else
				// $cycleInfo = array('prj'=>$prj, 'rel'=>$rel, 'creater'=>$creater_id, 'start_date'=>$date, 'end_date'=>$date, 'testcase_type_id'=>'TESTCASE_TYPE_PSDK');
			// $codec_import->import_testresult($cycleInfo, $data);
		}
	}
	...
	function uploadSingleLog($logfile, $elementId){//detail????
		// rename the file
	...
	function handleLogFile($cycle_id, $fileName){
		//是处理上传已有cycle_id的情况？？？？？
		$res = $this->db->query("SELECT *, prj.name as name, prj.os_id as os_id FROM cycle LEFT JOIN prj ON cycle.prj_id=prj.id WHERE cycle.id=".$cycle_id);
		$cycleInfo = $res->fetch();
		$relFileName = basename($fileName);
		if (stripos(strtolower($relFileName), '.zip') !== false){
			// analyze the file
