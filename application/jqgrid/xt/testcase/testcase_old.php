...
//print_r($this->options['gridOptions']['colModel']);	
    ...
    public function getMoreInfoForRow($row){
		if (1){//}!$this->prj_exist){ // 没有Project过滤项
			$sql = "SELECT group_concat(DISTINCT testcase_ver_id) as ver_ids, ".
				" group_concat(DISTINCT prj_id) as prj_ids, group_concat(distinct auto_level_id) as auto_level_ids, ".
				" group_concat(distinct testcase_priority_id) as testcase_priority_ids, group_concat(distinct owner_id) as owner_ids".
				" FROM prj_testcase_ver WHERE testcase_id=".$row['id'];
			if(!empty($this->prj_where))
				$sql .= " AND ".$this->prj_where;
			$res = $this->db->query($sql);
			$prj = $res->fetch();
			$row['ver_ids'] = $prj['ver_ids'];
			$row['prj_ids'] = $prj['prj_ids'];
			$row['auto_level_ids'] = $prj['auto_level_ids'];
			$row['testcase_priority_ids'] = $prj['testcase_priority_ids'];
			$row['owner_ids'] = $prj['owner_ids'];
		}
		if (!empty($row['ver_ids'])){
			$res = $this->db->query("SELECT group_concat(distinct command separator '\\n') as command FROM testcase_ver where id in ({$row['ver_ids']})");
			$command = $res->fetch();
	...
	protected function getSpecialFilters(){
		return array('os_id', 'board_type_id', 'chip_id', 'prj_id', 'edit_status_id', 'owner_id', 'testcase_priority_id', 'auto_level_id');
	}
	
	protected function specialSql($special, &$ret){
		$this->prj_exist = count($special);
		if ($this->prj_exist){
			$ret['main']['from'] .= " LEFT JOIN prj_testcase_ver FORCE INDEX(testcase_id_2) ON testcase.id=prj_testcase_ver.testcase_id";
			// $ret['main']['fields'] .= ", group_concat(distinct prj_testcase_ver.testcase_ver_id) as ver_ids, ".
				// "group_concat(distinct prj_testcase_ver.prj_id) as prj_ids, ".
				// "group_concat(distinct prj_testcase_ver.auto_level_id) as auto_level_ids, ".
				// "group_concat(distinct prj_testcase_ver.testcase_priority_id) as testcase_priority_ids";
			...
			foreach($special as $c){
				switch($c['field']){
					case 'prj_id':
						$prj_id = true;
						$this->prj_where = $this->tool->generateLeafWhere($c);
			...
			if (!$prj_id && $prj_where != '1'){
				...
				if (empty($row)){
					...
					$this->prj_where = $this->tool->generateLeafWhere(array('field'=>'prj_id', 'op'=>'IN', 'value'=>$row['ids']));
