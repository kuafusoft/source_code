...
class qygl_dingdan_cg extends table_desc{
	protected function init($db, $table, $params = array()){
// print_r($params);		
		parent::init($db, $table, $params);
		$info = array('unit_name'=>'?');
		if($this->actionName == 'information' || $this->actionName == 'update_information_page'){
			$sql = "SELECT unit.name as unit_name from wz left join unit on wz.unit_id=unit.id left join dingdan_cg on dingdan_cg.wz_id=wz.id where dingdan_cg.id={$params['id']}";
		...
		$this->options['edit'] = array('wz_id'=>array('editable'=>true), 'unit_name'=>array('type'=>'hidden'), 'defect_id'=>array('editable'=>true), 'price', 'amount', 'dingdan_status_id'=>array('type'=>'hidden'));
	}
	
	protected function setSubGrid(){
        $this->options['gridOptions']['subGrid'] = true;
		$this->options['subGrid'] = array('expandField'=>'dingdan_cg_id', 'db'=>'qygl', 'table'=>'dingdan_cg_jfjh');
	}
