...
//接收采购的货物
class qygl_yw_jh extends table_desc{
...
// print_r($params);
        ...
        $this->options['list'] = array(
			...
			'zzvw_cg_pici'=>array('label'=>'清单', 'editable'=>true, 'legend'=>'',
				'formatter'=>'multi_row_edit', 
				'formatoptions'=>array('subformat'=>'temp', 'temp'=>"%(gys_id)s发售的%(defect_id)s的%(wz_id)s %(amount)s")
				),
			'weight'=>array('label'=>'重量', 'post'=>array('value'=>'吨')),
			'dj_id'=>array('label'=>'单据'),
			'note'=>array('label'=>'备注'),
			'jbr_id'=>array('label'=>'收货人', 'data_source_db'=>'qygl', 'data_source_table'=>'yg'),
			'happen_date'=>array('label'=>'入库日期', 'edittype'=>'date',
				'stype'=>'select', 'searchoptions'=>array('value'=>$year_month2)), //只提供三年内的查询
			'*'=>array('hidden'=>true),
        );

		$this->options['add'] = array('cyr_id', 'yunshu_price', 'zxr_id', 'zx_price', 'zzvw_cg_pici', 'weight', 
			'dj_id', 'note', 'jbr_id', 'happen_date');
		$this->options['edit'] = array('cyr_id', 'yunshu_price', 'zxr_id', 'zx_price', 'zzvw_cg_pici', 'weight', 
			'dj_id', 'note', 'jbr_id', 'happen_date');
	...
	protected function setSubGrid(){
		...
		$this->options['subGrid'] = array('expandField'=>'yw_jh_id', 'db'=>'qygl', 'table'=>'zzvw_cg_pici');
	}
