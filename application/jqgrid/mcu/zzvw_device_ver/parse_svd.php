...
class svd_parse extends xml_parse{
	protected $tool = null;
...
// print_r($this->data);
		...
		$root = $this->data['root'];
		$cpu = isset($root['cpu']) ? $root['cpu'] : array();
...
// print_r($d);			
		...
		$device_ver['license_text'] = isset($device_ver['licenseText']) ? $device_ver['licenseText'] : '';
	...
	protected function addPeripherals($peripherals, $device_ids){
		foreach($peripherals['peripheral'] as $data){
			...
			$peripheral['prepend_to_name'] = isset($peripheral['prependToName']) ? $peripheral['prependToName'] : '';
...
// print_r($p_ver);
			...
			foreach($data as $register){
				$this->addRegister($register, $p_ver_id, $device_ids);
			}
			//处理interrupt
			if(!empty($interrupts)){
				...
				foreach($interrupts as $interrupt){
					$this->addInterrupt($interrupt, $p_ver_id, $device_ids);
				}
			}
		}
	}
	
	protected function addInterrupt($interrupt, $p_ver_id, $device_ids){
		$i_id = $this->tool->getElementId('interrupt', array('name'=>$interrupt['name']));
		$v = $interrupt;
		$v['interrupt_id'] = $i_id;
		unset($v['name']);
		$v_id = $this->tool->getElementId('interrupt_ver', $v);
		$this->tool->getElementId('interrupt_ver_peripheral_ver', array('peripheral_ver_id'=>$p_ver_id, 'interrupt_ver_id'=>$v_id, 'device_ver_id'=>$device_ids['device_ver_id']));
		$this->tool->getElementId('device_ver_interrupt_ver', array('device_ver_id'=>$device_ids['device_ver_id'], 'interrupt_ver_id'=>$v_id));
	}
	
	protected function addRegister($register, $p_ver_id, $device_ids){
		...
		$r_v['dim_index'] = isset($r_v['dimIndex']) ? $r_v['dimIndex'] : '';
		$r_v_id = $this->tool->getElementId('register_ver', $r_v);
		
		$this->tool->getElementId('peripheral_ver_register_ver', array('peripheral_ver_id'=>$p_ver_id, 'register_ver_id'=>$r_v_id, 'device_ver_id'=>$device_ids['device_ver_id']));
		$this->tool->getElementId('device_ver_register_ver', array('device_ver_id'=>$device_ids['device_ver_id'], 'register_ver_id'=>$r_v_id));
		if(!empty($fields))
			$this->addFields($fields, $r_v_id, $p_ver_id, $device_ids);
	}
	
	protected function addFields($fields, $r_v_id, $p_ver_id, $device_ids){
		...
		foreach($data as $field){
			...
			if(!empty($enumeratedValues))
				$this->addEnumeratedValues($enumeratedValues, $f_v_id, $r_v_id, $p_ver_id, $device_ids);
			$this->tool->getElementId('field_ver_register_ver', 
				array('field_ver_id'=>$f_v_id, 'register_ver_id'=>$r_v_id, 'peripheral_ver_id'=>$p_ver_id, 'device_ver_id'=>$device_ids['device_ver_id']));
			$this->tool->getElementId('device_ver_field_ver', array('device_ver_id'=>$device_ids['device_ver_id'], 'field_ver_id'=>$f_v_id));
		}
	}
	
	protected function addEnumeratedValues($enumeratedValues, $f_v_id, $r_v_id, $p_v_id, $device_ids){
		$data = $enumeratedValues['enumeratedValue'];
		if(!is_array(current($data)))
			$data = array($data);
		foreach($data as $v){
			$v['field_ver_id'] = $f_v_id;
			$v_id = $this->tool->getElementId('enumerated_value', $v);
			$this->tool->getElementId('device_ver_enumerated_value', 
				array('device_ver_id'=>$device_ids['device_ver_id'], 'enumerated_value_id'=>$v_id, 'field_ver_id'=>$f_v_id, 
				'register_ver_id'=>$r_v_id, 'peripheral_ver_id'=>$p_v_id));
		}
	}
}
...
// foreach($files as $file){
	// $svd_parse = new svd_parse('svd/'.$file);
	// $data = $svd_parse->parse();
// }
// $file = "mcu_header_file/CWMCUS_V10_6_Kinetis_50MHz_KM1x_KM3x_FINAL_DROP_140717/SVD/MKM33Z5.svd";
// $svd_parse = new svd_parse($file);
// $data = $svd_parse->parse();
// // print_r($data);
// $file = "mcu_header_file/CWMCUS_V10_6_Kinetis_50MHz_KM1x_KM3x_FINAL_DROP_140717/SVD/MKM34Z5.svd";
// $svd_parse = new svd_parse($file);
// $data = $svd_parse->parse();

// $file = "mcu_header_file/CWMCUS_V10_6_Kinetis_50MHz_KM1x_KM3x_FINAL_DROP_140717/SVD/MKM14Z5.svd";
// $svd_parse = new svd_parse($file);
// $data = $svd_parse->parse();

// $file = "SVD/MK24F25612.svd";
// $svd_parse = new svd_parse($file);
// $data = $svd_parse->parse();
