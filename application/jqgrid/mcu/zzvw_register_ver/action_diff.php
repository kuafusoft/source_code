...
class mcu_zzvw_register_ver_action_diff extends action_jqgrid{
...
// print_r($params);	
		...
		$f_res = $this->tool->query("select ver.*, f.name, device_ver_id ".
			" from device_ver_field_ver left join zzvw_field_ver ver on device_ver_field_ver.field_ver_id=ver.id".
			" left join field f on ver.field_id=f.id".
			" where device_ver_id in ($str_vers) order by name asc");
		while($f = $f_res->fetch()){
			foreach($f as $k=>$v)
