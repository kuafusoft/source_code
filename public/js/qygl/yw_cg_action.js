...
(function(){
	...
	var eventsBindForCG = function(divId, context, first){ //订单类事件绑定
		//交易方和物资之间绑定
		var prefix = 'dingdan_cg', temp = '#' + divId + ' #' + prefix + '_temp';
		target = [
			{
				selector:temp + ' #wz_id', 
				type:'select', 
				field:'gys_id', 
				url:'/jqgrid/jqgrid/oper/get_wz_by_gys/db/qygl/table/wz'
			}
		];
		tool.linkage({selector:'#' + divId + ' #gys_id'}, target);
		//物资和计量单位及默认单价绑定
